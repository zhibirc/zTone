...
For detailed explanation about various performance tips and pitfalls see [ptt](https://github.com/zhibirc/ptt)
...
- there is interesting and handful feature in ES2015 -- _arrow functions_. Well, their syntax is more terse than with `function` operator, 
but don"t hurry to replace all your "old" code with modern "sugar". There are several reasons to do this rationally:
 - clarity of intent. `function` operator from the start of the line demonstrates your intention more explicit;
 - order of declaration and order of execution. There are two different schemes here. _Function declarations_ can be seen 
 everywhere and immediately in their scope and _fat arrow functions_ as function expressions cannot;
 - displaying in call-stack/debugger;
 - handling `this`. Look at this:
 ```javascript
 (() => { return this.foo }).call({foo:1}); // undefined
 (function () { return this.foo }).call({foo:1}); // 1
 ```
